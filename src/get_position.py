...
def callback1(msg):
    ...
    # rospy.loginfo('x:{} , y:{},'.format(x2,y2))
...
while not rospy.is_shutdown():
    ...
    distance=math.sqrt((goal.x-pose_x)**2+(goal.y-pose_y)**2)
    # distance from robot to robot_1
    distance2=math.sqrt((pose_x-x2)**2+(pose_y-y2)**2)
    angledif=angle_to_goal-pose_theta
    # if abs(distance2)<1.5:
    if abs(distance>1.5) :       
        if abs(angledif)>=0.2 :
            if angledif!=0 :
